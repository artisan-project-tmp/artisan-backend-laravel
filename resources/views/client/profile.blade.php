...
<body class="antialiased bg-stone-50 text-stone-900 dark:bg-[#0c0a09] dark:text-[#d6d3d1] selection:bg-amber-700 selection:text-white transition-colors duration-300">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-20">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <span class="hidden sm:inline-block text-sm text-stone-500 dark:text-stone-400">Welcome, {{ explode(' ', $user->name)[0] }}</span>
                </div>
            </div>
        </div>
    </nav>
    ...
    <main class="pt-32 pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="glass-card rounded-2xl p-8 lg:p-12 shadow-sm mb-12 relative overflow-hidden">
            <div class="absolute -right-16 -top-16 w-64 h-64 bg-amber-500/10 dark:bg-amber-600/10 rounded-full blur-3xl text-gray-500"></div>
            <div class="flex flex-col md:flex-row gap-8 items-start relative z-10">
                <div class="w-full md:w-2/3">
                    <h1 class="font-heading text-4xl md:text-5xl font-bold text-stone-900 dark:text-stone-100">{{ $artisanUser->name }}</h1>
                    <p class="text-amber-700 dark:text-amber-500 text-sm font-bold tracking-widest uppercase mt-2 dark:glow-amber-text">{{ $artisanUser->artisan->craft_type }}</p>
                    <div class="mt-6">
                        <p class="text-stone-600 dark:text-stone-400 text-lg leading-relaxed content-bio mb-8">
                ...
                <div class="w-full md:w-1/3">
                    ...
                    @endphp
                    <div class="glass-card rounded-xl p-6 bg-stone-50 dark:bg-stone-900/40 border border-stone-200 dark:border-stone-800">
                        <div class="flex items-center justify-between mb-4 pb-4 border-b border-stone-200 dark:border-stone-800">
                            <div>
                                ...
                                <div class="font-heading text-2xl font-bold text-stone-900 dark:text-stone-100 flex items-center gap-2">
                                    {{ $avgRating }} @if($avgRating !== 'New')<span class="text-amber-500 text-xl shadow-amber-500">★</span>@endif
                                </div>
                            </div>
                            <div class="text-right">
                                <div class="text-2xl font-bold text-stone-900 dark:text-stone-100">{{ $reviewCount }}</div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <div class="relative group aspect-[4/5] rounded-xl overflow-hidden border border-stone-200 dark:border-stone-800 shadow-sm">
        ...
        <div class="relative w-full max-w-lg mx-4 glass-card rounded-2xl p-8 shadow-2xl z-10">
            ...
            <form action="{{ route('booking.store', $artisanUser->id) }}" method="POST">
                ...
                <div class="flex gap-4">
                    <button type="button" onclick="document.getElementById('quote-modal').classList.add('hidden')" class="flex-1 px-4 py-3 bg-stone-200 hover:bg-stone-300 dark:bg-stone-800 dark:hover:bg-stone-700 text-stone-800 dark:text-stone-200 text-xs font-bold uppercase tracking-widest rounded transition-colors">Cancel</button>
